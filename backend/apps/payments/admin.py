...
class PagoAdmin(admin.ModelAdmin):
    ...
    actions = ['aprobar_pago_seleccionado', 'rechazar_pago_seleccionado']
    ordering = ('-created_at',)
    readonly_fields = ('created_at', 'updated_at')

    @admin.action(description="Aprobar pagos seleccionados")
    def aprobar_pago_seleccionado(self, request, queryset):
        from .services import PagoService
        for pago in queryset:
            PagoService.confirmar_pago(pago.referencia_externa)
        self.message_user(request, "Los pagos seleccionados han sido aprobados y los inmuebles activados.")

    @admin.action(description="Rechazar pagos seleccionados")
    def rechazar_pago_seleccionado(self, request, queryset):
